this.sellsword_retires_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "A",
			Text = "[img]gfx/ui/events/event_64.png[/img]{Alors que vous êtes sur le chemin, vous rencontrez un homme assis à côté du chemin. Il est vêtu d\'une armure usée et une arme encore plus usée est posée sur ses genoux. Il vous regarde avec le plus mou des saluts.%SPEECH_ON%Bonsoir. Si vous n\'êtes pas des mercenaires, alors je n\'ai jamais mis le feu au pantalon de mon père.%SPEECH_OFF%Cela semble être une histoire intéressante en soi, mais vous demandez plutôt à l\'homme ce qu\'il fait au milieu de la route. Plus important encore, vous demandez à cet homme plutôt bien portant s\'il cherche du travail.%SPEECH_ON%Un travail ? Non. Je n\'en ai pas besoin. J\'ai déjà fait le coup de l\'épée et j\'en ai fini avec ça. Vous savez quoi, tenez.%SPEECH_OFF%Il commence à défaire son armure et la jette sur le sol devant vous.%SPEECH_ON%Prenez la. Je n\'ai plus besoin de cette vie. Prends l\'arme, aussi. Je laisse toute cette merde derrière moi. Vous devriez, aussi, mais je sais que vous ne le ferez pas. Pas avant qu\'il ne soit trop tard, en tout cas. Je vais parcourir la terre jusqu\'à ce que mes pieds soient réduits en bouillie. Quant à vous, bon vent.%SPEECH_OFF%Et juste comme ça, l\'étranger s\'en va.}",
			Image = "",
			List = [],
			Options = [
				{
