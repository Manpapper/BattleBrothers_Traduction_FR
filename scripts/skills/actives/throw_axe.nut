this.throw_axe <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "Lancer de Hache";
		this.m.Description = "Lance une hache vers la cible. Ne peut être utilisé si vous êtes attaqué en mêlée.";
	...
	function getTooltip()
	{
		...
		if (ammo > 0)
		{
			ret.push({
				...
				text = "Vous avez [color=" + this.Const.UI.Color.PositiveValue + "]" + ammo + "[/color] de haches restantes"
			});
		}
		else
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]Plus de haches restantes[/color]"
			});
		}
