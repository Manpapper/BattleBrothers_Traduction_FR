this.sleeping_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "Ce personnage est tombé dans un sommeil anormal et est incapable d\'agir. Il se réveillera après [color=" + this.Const.UI.Color.NegativeValue + "]" + (this.m.TurnsLeft - 1) + "[/color] tour(s), mais il peut également être réveillé de force par ses alliés et se réveillera dès qu\'il subira des dégâts, quelle que soit leur ampleur.\n\nPlus la détermination d\'un personnage est élevée, plus il a de chances de résister à l\'envie de dormir.";
	}
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-100[/color] d\'Initiative"
			},
			{
				id = 9,
				type = "text",
				icon = "ui/icons/special.png",
				text = "Peut subir des dégâts infligés par les Alps"
			}
		];
	}
	...
	function onAdded()
	{
		if (this.getContainer().getActor().getCurrentProperties().IsResistantToAnyStatuses && this.Math.rand(1, 100) <= 50)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " se réveille grâce à sa physiologie atypique");
			}
