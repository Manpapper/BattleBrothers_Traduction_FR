this.withered_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "Sous l\'effet d\'une magie noire, ce personnage s\'est flétri jusqu\'à prendre l\'apparence d\'un vieil homme. Cet effet s\'estompera après [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.TurnsLeft + "[/color] tour(s).";
	}
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-" + this.m.TurnsLeft * 25 + "%[/color] de Dégâts"
			},
			{
				id = 12,
				type = "text",
				icon = "ui/icons/initiative.png",
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-" + this.m.TurnsLeft * 25 + "%[/color] d\'Initiative"
			},
			{
				id = 12,
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-" + this.m.TurnsLeft * 25 + "%[/color] de Fatigue Maximum"
			},
			{
				id = 11,
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]-" + this.m.TurnsLeft * 5 + "[/color] de Récupération de Fatigue par tour"
			}
		];
	}
	...
	function onAdded()
	{
		...
		if (statusResisted)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " s\'est rapidement remis de ce coup dur grâce à sa constitution physique hors du commun");
			}
