this.lindwurm_acid_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "L\'armure de ce personnage a été aspergée du sang acide d\'un Lindwurm, qui la ronge désormais lentement pendant encore [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.TurnsLeft + "[/color] tour(s).";
	}
	...
	function getTooltip()
	{
		...
		if (!this.getContainer().getActor().getFlags().has("head_immune_to_acid"))
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]10%[/color] de l\'armure de la tête est perdue chaque tour"
			});
		}
		...
		if (!this.getContainer().getActor().getFlags().has("body_immune_to_acid"))
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]10%[/color] de l\'armure du corps est perdue chaque tour"
			});
		}
	...
	function onAdded()
	{
		if (this.getContainer().getActor().getCurrentProperties().IsResistantToAnyStatuses && this.Math.rand(1, 100) <= 50)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " n\'est pas affecté par l\'acide grâce à sa physiologie atypique");
			}
		...
		else
		{
			...
			if (!this.getContainer().getActor().isHiddenToPlayer())
			{
				this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " n\'est pas affecté par l\'acide grâce à sa physiologie atypique");
			}
		}
	}
