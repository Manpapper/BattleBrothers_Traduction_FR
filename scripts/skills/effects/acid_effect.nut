this.acid_effect <- this.inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		if (!this.getContainer().getActor().getFlags().has("head_immune_to_acid"))
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]20%[/color] de l\'armure de la tête est perdue chaque tour"
			});
		}
		...
		if (!this.getContainer().getActor().getFlags().has("body_immune_to_acid"))
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]20%[/color] de l\'armure du corps est perdue chaque tour"
			});
		}
	...
	function onAdded()
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + " a eu son corps aspergé d\'acide");
		}
	}
