this.skeleton_warrior_potion_item <- this.inherit("scripts/items/misc/anatomist/anatomist_potion_item", {
	...
	function create()
	{
		...
		this.m.Name = "Potion du Veilleur sans Repos";
		this.m.Description = "En buvant ceci, même le mercenaire le plus faible obtiendra un bras bouclier qui rivalise même avec celui des morts inlassables ! Presque garanti de ne pas les transformer en morts-vivants, non plus !";
		this.m.IconLarge = "";
	...
	function getTooltip()
	{
		...
		result.push({
			...
			text = "Le coût de Fatigue de la compétence \'Mur de Boucliers\' est réduit de [color=" + this.Const.UI.Color.PositiveValue + "]" + 100 * (1 - this.Const.Combat.WeaponSpecFatigueMult) + "%[/color]"
		});
		result.push({
			id = 65,
			type = "text",
