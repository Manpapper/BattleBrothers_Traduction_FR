this.siege_fortification_contract <- this.inherit("scripts/contracts/contract", {
   ...
   "Image": ""
		...
		this.m.Screens.push({
			ID = "DefendersPrevail",
			Title = "Après la bataille...",
			Text = "[img]gfx/ui/events/event_22.png[/img] Incroyablement, les défenseurs épuisés de %objectif% ont gagné ! Vous vous retirez alors que le siège s'effondre.",
			Image = "",
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			...
			Title = "Après la bataille...",
			Text = "[img]gfx/ui/events/event_31.png[/img]{Les défenseurs de %objectif% ont été anéantis et les fortifications laissées grandes ouvertes. Vous et %commandant% traversez les portes ouvertes pour trouver des cadavres, des débris et des animaux abattus partout, des signes sanglants de désespoir. Le commandant hoche la tête et vous tape sur l'épaule.%SPEECH_ON%Bon travail, mercenaire. Vous devriez retourner maintenant chez %employer% et lui annoncer la nouvelle.%SPEECH_OFF% | La bataille est terminée, les défenseurs de %objectif% complètement vaincus et leur fort laissé grand ouvert à la prise. %commandant% vous remercie pour vos services avant de libérer les services des %companyname% du champ de bataille. Vous devriez maintenant aller voir un %employer% très heureux. | C'était un effort plein d'ardeur de la part des défenseurs de %objectif%, mais s'ils voulaient tenter quelque chose comme ça, ils auraient dû le faire il y a des semaines, lorsque leurs forces correspondaient à leur héroïsme. Peu importe maintenant. Un homme mort affamé ressemble beaucoup à un homme mort bien nourri et, avec assez de temps, ils se ressemblent tous.\n\n %commandant% vient vous dire que les services de %companyname% ne sont plus nécessaires. Vous êtes d'accord et devriez retourner chez %employer% pour le paiement. | S'aventurer dehors lorsque l'on est affamé et assiégé par un mauvais leadership n'est jamais la meilleure des idées. Vous n'êtes pas sûr que %commandant% aurait accordé la clémence aux défenseurs de %objectif% s'ils s'étaient rendus. Tels qu'ils sont maintenant, ils sont tous morts dans la boue, et le monde dans lequel ils se sont rendus a depuis longtemps disparu. Vous rassemblez les hommes de %companyname% et leur ordonnez de se préparer pour le retour chez %employer%. La paie sera bien douce après cette journée. | Avec les défenseurs de %objectif% écartés, vous et %commandant% entrez dans les fortifications. Il y a une raison pour laquelle les hommes étaient si désespérés : les conditions sont absolument déplorables. Des hommes morts ont été dépouillés et entassés dans un coin. Une broche tient les restes cuits de ce qui aurait pu être un cochon, mais il est difficile de le dire car ils ont tout simplement consommé chaque morceau de cet animal. Un homme pendu balance depuis l'une des tours. Ils avaient cloué une planche à sa poitrine avec 'cannibale' écrit dessus, probablement écrit avec l'utilisation de son propre sang.\n\n %commandant% rit.%SPEECH_ON%On dirait une vraie fête ici, n'est-ce pas ? Souvenez-vous de cette scène la prochaine fois qu'un lieutenant belliqueux avec une idée fixe vous dira de continuer à tenir bon.%SPEECH_OFF% | Les %companyname% et l'armée de %commandant% ont définitivement vaincu les défenseurs qui sortaient de %objectif%. Avec les fortifications laissées libres, les hommes de %commandant% les prennent rapidement en charge. Le commandant lui-même vous dit d'aller voir %employer% pour le paiement. | Les défenseurs de %objectif% sont morts sur un champ de bataille, mais c'était plutôt un champ de clémence. Derrière les murs de leur fort, il ne reste presque rien de valeur et, en particulier, une absence totale de nourriture. On dirait que le monde qui existait derrière les murs n'a même jamais connu la nourriture, les défenseurs ayant tellement nettoyé l'endroit. Vous êtes sûr que de simples mentions de nourriture étaient un crime, car même un mot torturé par une saveur serait comme un fouet sur l'estomac grognant d'un homme. %commandant% vient à vos côtés et rit.%SPEECH_ON%Je pensais savoir ce que c'était d'avoir faim, mais j'avais toujours une solution, vous savez ? Je n'ai jamais eu faim sans espoir de le résoudre. Quelle chose horrible. Mais encore une fois, ils l'ont résolu, n'est-ce pas ?%SPEECH_OFF%Vous hochez la tête alors que l'homme rit de son humour sombre.%SPEECH_ON%Vous avez fait du bon travail, mercenaire. Allez, assurez-vous que %employer% vous paie bien.%SPEECH_OFF%}",
			Image = "",
			List = [],
			Options = [
				{
					Text = "%objectif% est tombé !",
					function getResult()
					{
						this.Contract.changeObjectiveOwner();
						this.Contract.setState("Return");
		...
		this.m.Screens.push({
			ID = "Prisoners",
			Title = "Après la bataille...",
			Text = "[img]gfx/ui/events/event_53.png[/img]{Quelques-uns de vos hommes ont réussi à capturer certains des défenseurs de %objectif%. Ils se tiennent là, regroupés, entourés par les armes de votre mercenaire. Certains tremblent dans leurs bottes. Un n'a même pas de bottes. Un autre a des taches sur son pantalon. %randombrother% demande ce qu'il faut faire d'eux. | %randombrother% signale que quelques défenseurs de %objectif% ont été capturés. Vous allez trouver un groupe d'hommes regroupés, enlacés dans une étreinte circulaire, mais la tête baissée. L'un appelle.%SPEECH_ON%S'il vous plaît, ne nous tuez pas ! Nous faisions simplement ce que l'on nous disait, tout comme vous !%SPEECH_OFF% | Vos hommes ont réussi à capturer quelques-uns des défenseurs de %objectif%. Ils ont été rassemblés, dépouillés jusqu'à leur pantalon, et ordonnés de se coucher dans la boue. %randombrother% demande ce qu'il faut faire d'eux.}",
			Image = "",
			List = [],
			Options = [
				{
				...
				{
					Text = "Ils peuvent valoir quelque chose. Emmenez-les en tant que prisonniers auprès de %commandant%.",
					function getResult()
					{
