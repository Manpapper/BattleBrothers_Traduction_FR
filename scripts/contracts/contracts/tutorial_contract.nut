this.tutorial_contract <- this.inherit("scripts/contracts/contract", {
	...
	function createScreens()
	{
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_21.png[/img]Tout est parti en vrille. Deux jours plus tôt la compagnie a été engagée pour traquer %boss% et sa bande de voleurs, mais ce sont eux qui nous ont trouvés en premier. Une embuscade. Des blagues sur les chevaux, coupées court par une flèche dans la gorge. Des flèches venant de partout et nulle part en même temps. Des hommes hurlant, un doux son face à la mort.\n\nEn voyant la pluie de flèches s\'arrêter, vous sortez votre arme et vos compagnons font de même, enfin ceux qui sont encore vivants... Vous tombez à genoux. Une flèche a percé votre flanc. Vous hurlez de douleur. Un regard rapide où vous voyez vos hommes charger sans vous dans un dernier effort, l\'acier se heurt à l\'acier.\n\nVos yeux et ceux du capitaine se rencontrent, un dernier acquiescement de la tête avant que sa gorge ne soit tranchée. Vous êtes laissé aux commandes des derniers hommes encore en vie. Tremblant de douleur, vous vous appuyez sur votre épée et avec toute la détermination que vous pouvez rassembler vous vous relevez encore...",
			Image = "",
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_79.png[/img]Quel spectacle désolant pour les villageois lorsque vous arrivez à %townname%. Quatres mercenaires ensanglantés et fatigué qui n\'ont pas eu de chance. L\'homme qui vous a embauché deux jours plus tôt, %employer%, sans aucun doute s\'attendait à un retour plus glorieux.\n\nCependant, il vous accueille chez lui et vous offre du pain et du vin pendant qu\'un serviteur va chercher un soigneur. Peu de mots sont échangés à part quelques grognements occasionnels et la respiration du vieil homme avec les mains tremblantes qui s\'occupe de vos blessures. Une aiguille perce votre peau, la première d\'une longue série. Vous serrez les dents jusqu\'à avoir l\'impression que l\'une d\'entre elles se brise. %employer% s\'assoit à côté de vous et vous demande si vous vous êtes bien occupé de Hoggart. Vous secouez la tête.%SPEECH_ON%On a tué ses hommes, mais cette fouine a réussit à échapper à nos épées à la fin.%SPEECH_OFF%Le soigneur agite une arme de métal incandescente, suggérant qu\'il veut l\'appliquer sur la plaie. Vous hochez de la tête et il se mit à l\'oeuvre. Vous n\'êtes pas un homme, mais une pique de flamme, peau contre feu, un avatar de la douleur. %employer% vous tends un gobelet de vin.%SPEECH_ON%Vous avez bien travaillé, mercenaire. Les brigands ne sont plus, malgré le fait que Hoggart soit toujours en vie.%SPEECH_OFF%",
			Characters = [],
			ShowEmployer = true,
			List = [],
			Options = [
				{
					Text = "Nous espérions être payés pour cela.",
					function getResult()
					{
		...
		this.m.Screens.push({
			...
			Title = "Retournez à %townname%",
			Text = "[img]gfx/ui/events/event_79.png[/img]%employer% vous regarde.%SPEECH_ON%Oui bien sûr, naturellement! 400 Couronnes comme convenu.%SPEECH_OFF%Il fait signe à un servant qui s\'empresse de venir vers vous avec la bourse dans ses mains.%SPEECH_ON%Je me demandais... puis-je encore faire appel à vos services encore une fois? J\'aimerais vraiment être libéré du problème que représente Hoggart une bonne fois pour toute. Et je vous paierai de nouveau, bien sûr. 400 Couronnes, qu\'en dites vous?%SPEECH_OFF%%bro2% rigole et se tourne pour boire plus de vin, mais %bro1% se lève pour parler.%SPEECH_ON%Oui, la compagnie est en ruine, mais nous allons la reconstruire! Sans %companyname%, %bro2% boira les couronnes et finira par mendier dans les rues, et %bro3%, par les dieux nous savons tous qu\'ils passera son temps après les femmes jusqu\'à ce que l\'une d\'entre elle finisse par lui mettre la tête dans un four. On a besoin de %companyname%, c\'est tout ce que nous avons! Qu\'en pensez vous capitaine?%SPEECH_OFF%%bro2% rote puis lève son verre vers vous. %bro3% pose sa main sur son coeur et acquiesce.%SPEECH_ON%Que l\'on tue ce salaud d\'Hoggart ou non, ça ne dépend que de vous capitaine.%SPEECH_OFF%",
			Characters = [],
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			...
			Title = "Retournez à %townname%",
			Text = "[img]gfx/ui/events/event_79.png[/img]%employer% frappe dans ses main de bonheur.%SPEECH_ON%Excellent! Mes hommes vont mettre un peu de temps à trouver où Hoggart se cache. Pendant ce temps, je vous suggère de faire le plein de provisions pour que vous soyez prêt quand le moment viendra. On se revoit d\'ici quelques jours au plus tard!%SPEECH_OFF%En quittant la maison de %employer% et en vous rendant en bordure de %townname%, %bro1% souhaite vous parler.%SPEECH_ON%Nous avons besoin de plus d\'hommes capitaine. Je sais que j\'ai donné un beau discours tout à l\'heure mais la bravoure n\'y changera rien. Nous avons besoin de plus de personnes dans les rangs. Trois hommes supplémentaires serait un bon début, achetez quelques armes décentes et équipez-les des meilleures armures que nous pouvons nous permettre d\'acheter.%SPEECH_OFF%L\'homme s\'arrête et regarde autour.%SPEECH_ON%Je suis sûr que dans cette ville perdue, nous trouverons un ou deux paysans en quête d\'une nouvelle vie. Sinon nous pourrions aller jusqu\'à %bigcity% %citydirection%. Les gens de cette ville ne seront probablement pas aussi endurcis que ces gens de la campagne, mais on aura probablement plus de chance de tomber sur des hommes expérimentés qui se sont arrêtés là-bas pour se reposer.%SPEECH_OFF%",
			List = [],
			Characters = [],
			Options = [
				{
