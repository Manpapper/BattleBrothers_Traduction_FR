this.return_item_contract <- this.inherit("scripts/contracts/contract", {
	...
	function start()
	{
		...
		local items = [
			...
			"le Bâton de Cérémonie",
			"l\'Idole de la Fertilité",
			"le Talisman en Or",
			"le Tome de Connaissance des Arcanes",
			"le Coffre",
			"la Statuette Démoniaque",
			"le Crâne de Cristal"
		];
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "BattleDone",
			Title = "Après la bataille...",
			Text = "[img]gfx/ui/events/event_22.png[/img]{La bataille terminée, vous récupérez %itemLower% des griffes décharnées de vos ennemis et vous vous préparez à retournez voir %employer%. Il sera sûrement heureux de voir votre succès ! | Ceux qui ont volé %itemLower% sont morts, et heureusement vous avez pu trouver l\'objet lui-même. %employer% sera très heureux de votre travail ici. | Vous avez trouvé les responsables du vol, %itemLower% a été retrouvé(e) et vous avez passé les responsables au fil de l\'épée. Maintenant, il ne vous reste plus qu\'à remettre %itemLower% entre les mains de %employer% et obtenir votre récompense ! | La bataille est terminée et %itemLower% a été facile à trouver parmi les cadavres de vos ennemis. Vous devriez probablement le rendre à %employer% pour votre juste récompense !}",
			Image = "",
			List = [],
			Options = [
				{
